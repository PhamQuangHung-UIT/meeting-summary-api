...
def get_recording(recording_id: str, user_id: str = Depends(get_current_user)):
    recording = RecordingService.get_recording_details(user_id, recording_id)
    if not recording:
        raise HTTPException(status_code=404, detail="Recording not found")
    return recording

@router.post("/", response_model=schemas.Recording, status_code=status.HTTP_201_CREATED)
def create_recording(recording: schemas.RecordingInitRequest, user_id: str = Depends(get_current_user)):
    return RecordingService.create_recording_metadata(user_id, recording)

@router.post("/{recording_id}/complete-upload", response_model=schemas.Recording)
def complete_upload(recording_id: str, request: schemas.RecordingUploadCompleteRequest, user_id: str = Depends(get_current_user)):
    return RecordingService.complete_upload_recording(user_id, recording_id, request)
...
def patch_recording(recording_id: str, recording: schemas.RecordingUserUpdate, user_id: str = Depends(get_current_user)):
    return RecordingService.update_recording_details(user_id, recording_id, recording)

@router.delete("/{recording_id}", status_code=status.HTTP_204_NO_CONTENT)
def soft_delete_recording(recording_id: str, user_id: str = Depends(get_current_user)):
    RecordingService.soft_delete_recording(user_id, recording_id)
    return None

@router.post("/{recording_id}/restore", response_model=schemas.Recording)
def restore_recording(recording_id: str, user_id: str = Depends(get_current_user)):
    return RecordingService.restore_recording(user_id, recording_id)

@router.delete("/{recording_id}/hard-delete", status_code=status.HTTP_204_NO_CONTENT)
def hard_delete_recording(recording_id: str, user_id: str = Depends(get_current_user)):
    RecordingService.hard_delete_recording(user_id, recording_id)
    return None
...
def get_recording_speakers(recording_id: str, user_id: str = Depends(get_current_user)):
    return RecordingService.get_speakers(user_id, recording_id)
...
def update_recording_speaker(
    ...
    return RecordingService.update_speaker(user_id, recording_id, speaker_label, speaker_update)

@router.get("/{recording_id}/markers", response_model=List[schemas.Marker])
def get_recording_markers(recording_id: str, user_id: str = Depends(get_current_user)):
    # Verify access (optional but good practice)
    RecordingService.get_recording_details(user_id, recording_id) 
...
def create_export_job(
    ...
    RecordingService.get_recording_details(user_id, recording_id)
    try:
        return ExportJobService.create_export_job(user_id, recording_id, request.export_type)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
